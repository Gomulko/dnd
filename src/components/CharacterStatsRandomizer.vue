<template>
    <div class="container character-stats-randomizer">
      <div class="row justify-content-around">
        <div class="col-auto text-center" v-for="stat in generatedStats" :key="stat.id">
          <div class="stat" :class="{ 'crossed-out': isStatSelected(stat.id) }">{{ stat.total }}</div>
          <div class="rolls">
            <span v-for="(roll, rollIndex) in stat.rolls" :key="rollIndex" :class="{ 'crossed-out': rollIndex === stat.lowestIndex }">{{ roll }}</span>
          </div>
        </div>
      </div>
  
      <div class="row justify-content-around mt-4">
        <div class="col-auto text-center" v-for="(label, index) in labels" :key="'label-' + index">
          <div class="selected-stat">{{ getSelectedStatValue(index) }}</div>
          <select class="form-select" v-if="!isValueSelected(index)" v-model="selectedValues[index]">
            <option disabled value="0">Wybierz wartość</option>
            <option v-for="stat in availableStats" :key="stat.id" :value="stat.id">{{ stat.total }}</option>
          </select>
          <div>{{ label }}</div>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const labels = ['Siła', 'Zręczność', 'Kondycja', 'Inteligencja', 'Mądrość', 'Charyzma'];
  const selectedValues = ref<number[]>(new Array(6).fill(0));
  
  const availableStats = computed(() => {
    return generatedStats.value.filter(stat => !selectedValues.value.includes(stat.id));
  });
  ...
  const getSelectedStatValue = (index: number) => {
    const stat = generatedStats.value.find(stat => stat.id === selectedValues.value[index]);
